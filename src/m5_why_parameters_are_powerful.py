...
def main():
    ...
    even_better_draw_circles(rg.Point(0,0),7,15,'red', 6)
    even_better_draw_circles(rg.Point(-200, 200), 6, 5, 'yellow', 3)
    even_better_draw_circles(rg.Point(200, 200), 8, 25, 'blue', 17)
    even_better_draw_circles(rg.Point(-200, -200), 9, 10, 'green', 14)
    even_better_draw_circles(rg.Point(200, -200), 5, 7, 'orange', 13)
...
def even_better_draw_circles(point, amount, radii, clr, thic):
    ...
    turtle = rg.SimpleTurtle()
    turtle.pen.color = clr
    turtle.pen.thickness = thic
    turtle.pen_up()
    turtle.go_to(point)
    turtle.set_heading(0)  # Point "east" (towards the right)

    for k in range(1, amount):  # k becomes 1, 2, 3, ... 10
